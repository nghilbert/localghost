...
- **Test complex work:** for real moving parts (parsers, data transforms, non-trivial UI), write Vitest tests colocated with their subject (see Testing) asserting real behavior. Keep test data inline or tiny; craft the minimal input, never commit captured blobs.
...
Copy `.env.example` to `.env`. Required: `POSTGRES_PASSWORD`, `BETTER_AUTH_SECRET`, `ENCRYPTION_KEY` (64-char hex). `DATABASE_URL` is a single `.env` line interpolated from `POSTGRES_*` by dotenvx; DB-touching scripts run through `dotenvx run`. Optional: `SEARXNG_URL` (unset disables web search with a guidance message); `CODE_AGENT_WORKSPACE_ROOT`, the tree code-agent sessions may be created under, defaulting to the user's home directory.

## Architecture

- **Framework:** TanStack Start (SSR React, Vite + Hono); file-based routing. Alias `#/` maps to `src/`.
- **Routes = the pages layer.** `_authenticated.tsx` guards auth and renders the colocated `AppSidebar` shell. A route file owns the `Route` export (loader/head/search) and its page component `<RouteSubject>Page` (`library/index.tsx` defines `LibraryPage`). Pathless layout routes own shared geometry or behavior without changing public URLs (`_authenticated/_chat/route.tsx` wraps `/new` and `/chat/$id`). Code local to a route lives in kind-based `-`-prefixed segments hidden from route gen (`-components/`, `-hooks/`, `-lib/`): beside a single route file (`library/index.tsx` + `library/-components/`), inside a route directory (`settings/index.tsx` + `settings/-components/`), or under the **common route ancestor** that shares it (`_authenticated/_chat/-components/` serves the chat subtree; `_authenticated/-components/AppSidebar/` serves every authenticated page). A `-components/` folder is fine with a single member. Server-only logic shared by sibling API route handlers is one `-<name>.server.ts` file (`api/backup/-backup.server.ts`).
...
- **LLM:** `src/shared/lib/llm/client.server.ts`: `streamLLMEvents(opts)` returns `AsyncIterable<StreamChunk>`; pass `tools: ServerTool[]` to run the agent loop. A data-driven `PROVIDERS` registry handles per-provider URL/header/model-list/options quirks, provider auto-detected from URL. Wraps `@tanstack/ai`'s `chat()`.
...
- Route-level data loads through shared `queryOptions()` used by both the route loader and `useQuery`; components never ad-hoc `fetch`. Every route with queries defines a loader: an awaited `context.queryClient.query({ ...options, staleTime: "static" })` for fast first-paint data, dropping `staleTime` where the value has to refetch on revisit.
...
src/
  ...
  routes/          # TanStack routing == the pages layer; each route file owns Route + its <X>Page component
    ...
    _authenticated/
      -components/AppSidebar/  -components/ChatInput/  -components/ChatMessage/  -components/ChatStatus.tsx  -hooks/  # shared by every authenticated page (ChatInput/ChatMessage/ChatStatus, use-step-duration, use-endpoint-model-groups)
      _agent/                            # pathless layout for the code agent (/agent, /agent/$id)
        route.tsx  agent/index.tsx  agent/new.tsx  agent/$sessionId.tsx  agent/-components/
      ...
      library/index.tsx  library/-components/ library/-lib/  # the Library page UI
...
- **File suffixes are build boundaries.** Only `.server.*` and `.client.*` are enforced, by TanStack's import-protection globs (`**/*.server.*` denied in the client bundle, `**/*.client.*` denied in the server bundle). `*.functions.ts` (the `createServerFn` RPC boundary) is a documented TanStack convention that no tooling reads; it must stay isomorphic, since the client imports it for the RPC stub and its `queryOptions`. Also `types.ts` for types, `schemas.ts` for Zod.
- **A boundary suffix needs a word in front.** `server.ts` and `client.ts` contain `server.`/`client.` but not `.server.`/`.client.`, so they fall outside the globs and get **no protection at all**, silently: no error, no type failure. Always `<role>.server.ts`. The same trap catches `server.functions.ts`.
- **The suffix describes what a module touches when it loads, not who calls it.** Server-only (`<role>.server.ts`) touches prisma, secrets, or `node:` APIs at its top level. Browser-only (`<role>.client.ts`) touches `window`, `document`, or `localStorage`. Isomorphic (a plain hyphenated name) touches neither, and is the default. `.client.ts` is allowed for genuinely browser-only modules, consumed via `<ClientOnly fallback={...}>` or `createClientOnlyFn()`; never put it on an isomorphic module, which is why better-auth's client is `auth-client.ts` and not `auth.client.ts`.
### Naming

The mechanical rules (camelCase Zod schemas, stem-less boundary filenames) are enforced by `.claude/hooks/post-edit.ts`. The judgment calls it cannot decide:
...
- **Files: the folder owns the noun, the file owns the role.** A file never repeats the noun its folder already states: `memory/adapter.server.ts`, not `memory/memory-adapter.server.ts`. Two exceptions, both TanStack's documented shape (`users.functions.ts` / `users.server.ts`): inside `shared/domain/<noun>/` the RPC boundary stays `<noun>.functions.ts` and the primary data-access module stays `<noun>.server.ts`. Everywhere else (`shared/lib/<thing>/`, route `-lib/`) is role-named with no echo (`lib/llamacpp/client.server.ts`, `lib/llm/provider.ts`).
- **Role names state purpose, not essence.** No `utils.ts`, `helpers.ts`, `hooks.ts`, or `misc.ts` as a dumping ground. `use-<noun>.ts` keeps its noun, since the `use-` prefix needs one to attach to; drop only the folder's own noun when a more specific one exists (`code-agent/use-sessions.ts`) and pluralize for collections.
...
Vitest tests are colocated: a test file sits beside the subject it tests, named for it exactly (`memory/adapter.server.ts` → `memory/adapter.server.test.ts`, `ChatInput/index.tsx` → `ChatInput/ChatInput.test.tsx` for a folder component). `src/test/` holds only what has no single subject: `factories.ts`, `msw.ts`, `utils.tsx` (shared test infra), and `integration/` for the handful of cross-module flow tests (`chat-stream.test.tsx`, `agent-stream.test.tsx`) that exercise a connection adapter through `useChat` end-to-end rather than one file. Run with `npm run test -- run` (see the "Test complex work" rule for _when_). Two projects split by extension: `*.test.ts` runs in node (`unit`), `*.test.tsx` in headless Chromium via browser mode (`browser`), both globbed from all of `src/`. Browser tests use `render`/`renderHook` from `#/test/utils` (wraps `vitest-browser-react`; both async), interactions via locators (`await screen.getByTestId(...).click()`) or `userEvent` from `vitest/browser`, assertions via `await expect.element(...)` / `expect.poll`. `.claude/hooks/post-edit.ts` enforces the mechanical patterns (userEvent over `fireEvent`, no casts, query by `data-testid` not role/label/text). The judgment:
...
- **One test file per subject.** A test covering several components (a shared mock, similar setup) still splits one file per component, so the colocated pairing stays 1:1: `SignInForm.tsx`/`SignInForm.test.tsx` and `SignUpForm.tsx`/`SignUpForm.test.tsx`, not one `auth-forms.test.tsx` for both. The `use-app-form` kit is the deliberate exception: the hook, its form components, and its field components are documented as one kit, so `use-app-form/use-app-form.test.tsx` tests them together.
- A route-colocated test takes the same `-` exclusion as its subject (`api/backup/-backup.server.ts` → `-backup.server.test.ts`), so it stays out of route generation.
...
| Library (core) | data `src/shared/domain/model/` (`catalog.server.ts` reads the Hugging Face GGUF index via `huggingface.server.ts`, `hardware.server.ts` probes the host, `hardware-fit.ts` scores fit, `model.functions.ts`), page UI `src/routes/_authenticated/library/index.tsx` + `library/-components/ModelList/`: browse and install local models |
...
| Code agent | `src/shared/domain/code-agent/` (`run.server.ts` runs Claude Code via `localProcessSandbox` directly against the session's `workspacePath`, `policy.server.ts` builds the sandbox policy and renames the approval chunk, `env.server.ts` scrubs the inherited environment by inversion, `compatibility.server.ts` refuses a model whose chat template can't drive a harness), page UI `src/routes/_authenticated/_agent/`. `harnesses.ts` is the harness registry, pinned to the endpoint providers whose wire protocol it speaks and the CLI it needs on PATH; models come from the chosen endpoint, never the registry. The session id doubles as the AG-UI `threadId`, so `/api/agent/stream` reads `params.threadId`; its only forwarded props are the command approvals granted for that one run, which are never stored (an `allow` entry is matched as a glob, so a stored one would over-grant). Under Compose `CODE_AGENT_WORKSPACE_ROOT` (default `os.homedir()`, i.e. the whole home directory) is mounted at its host path, so a workspace path resolves the same inside and out. Edits land in place; the approval gate and Claude Code's own Bash sandbox stand in for container isolation |
