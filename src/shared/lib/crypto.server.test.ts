...
describe("crypto.server", () => {
	describe("encrypt", () => {
		it("should return a string in iv:tag:ciphertext format", async () => {
			...
			const { encrypt } = await import("#/shared/lib/crypto.server");
	...
	describe("decrypt", () => {
		it("should round-trip plaintext", async () => {
			process.env.ENCRYPTION_KEY = VALID_KEY;
			const { encrypt, decrypt } = await import("#/shared/lib/crypto.server");
		...
		it("should throw on invalid ciphertext format", async () => {
			process.env.ENCRYPTION_KEY = VALID_KEY;
			const { decrypt } = await import("#/shared/lib/crypto.server");
