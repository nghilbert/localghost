...
function session(id: string, updatedAt: string) {
	return {
		id,
		title: id,
		workspacePath: `/srv/${id}`,
		updatedAt: Temporal.PlainDateTime.from(updatedAt),
	};
}
...
describe("sortSessionsByActivity", () => {
	it("orders by the transcript's timestamp when it is newer than the row's", () => {
		...
		const threadActivity = new Map([["a", Temporal.PlainDateTime.from("2026-03-01")]]);
	...
	it("keeps the row's own timestamp when the transcript is older", () => {
		...
		const threadActivity = new Map([["a", Temporal.PlainDateTime.from("2026-01-01")]]);

		expect(sortSessionsByActivity({ sessions, threadActivity })[0]?.updatedAt.toString()).toEqual(
			Temporal.PlainDateTime.from("2026-03-01").toString(),
		);
	});
