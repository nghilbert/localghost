...
/**
 * Whether a model can use tools and accept images. Tool support: llama.cpp does
 * grammar-constrained tool calling for essentially any model, so local runtimes
 * always report `true`; cloud providers go through {@link modelSupportsTools}.
 * Unknown cases are assumed capable, never wrongly blocking a working model.
 * Image support: llama.cpp reports it via `GET /models`'s `input_modalities`;
 * cloud providers can't be probed, so we stay permissive and let the model
 * reject an image it can't read.
 */
